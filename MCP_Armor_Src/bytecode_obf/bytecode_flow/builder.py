...
from MCP_Armor_Src.bytecode_obf.bytecode_flow.model import BasicBlock, ControlFlowGraph, Edge, Instruction
...
def _read_arg(code_bytes, offset):
    return byte_value(code_bytes[offset + 1]) | (byte_value(code_bytes[offset + 2]) << 8)
...
def decode_instructions(code_bytes):
    ...
    size = len(code_bytes)
    while offset < size:
        opv = byte_value(code_bytes[offset])
...
def build_control_flow_graph(code_bytes, extra_leaders=None):
    ...
    # Materialize edge-local stack facts once.  All ByteCode_Flow transforms
    # can now use the same compatibility gate instead of reimplementing the
    # conditional/iterator stack rules independently.
    for edge in edges:
        source = edge.source.terminator
        edge.stack_in = depths.get(source.offset)
        edge.stack_out = depths.get(edge.target.start)
        edge.stack_effect = instruction_stack_effect(source, edge.kind)
        if (edge.stack_in is not None and edge.stack_out is not None and
                edge.stack_effect is not None):
            edge.stack_compatible = (
                edge.stack_in + edge.stack_effect == edge.stack_out and
                edge.stack_out >= 0)
