# -*- coding: utf-8 -*-
"""Per-basic-block flow predicates for verified CPython 2.7 CFGs."""
...
from MCP_Armor_Src.bytecode_obf.cfg.relocator import (
    RelocationLayout,
    encode_oparg,
    relocate_stream,
)
...
def apply_block_seed_flow(co, code_bytes, consts, varnames,
    ...
    """Propagate a unique seed across every ordinary edge in a function CFG."""
    ...
    try:
        transformed, rendered_layout = relocate_stream(
            graph, prefix, before, after, jump_overrides, ''.join(suffix))
